...
crates/server/src/
  ...
  business/wordbooks/words.rs + words/ /api/wordbooks/{book_id}/words... (router/service/repo/dto/error/import/order/sort/sort_dir/tag_match/file_type/import_filter/pos/template_format)
...
- **Closed sets are enums:** query params (`order`, `seed`, `sort`, `tag_match`) are parsed once at the router boundary into `WordOrder` / `SortField` / `SortDir` / `TagMatch`; business logic matches enum variants, never string literals. `SortField`/`SortDir`/`TagMatch` use `strum::EnumString` (declarative mapping); `WordOrder` keeps a handwritten parse because `Random(String)` carries a payload strum cannot express. `TagMatch` (and/or) 走 `tag_match` 查询参数，缺省 `and`（向后兼容）；洗牌缓存 key 直接使用 `TagMatch` 枚举分量（`common::state` 已依赖 business DTO 类型，不再字符串化）。请求体/查询里的封闭集合同此处理：`ImportFilter`（导入行筛选，serde `deserialize_with` + TS union 导出）、`TemplateFormat`（模板下载格式）、`ImportFileType`（导入文件格式）、`WordPos`（词性白名单）。
...
- `AppState` holds the DB pool, the full `Config`, and three process-memory caches: `wordbooks_cache` (list result, invalidated via `state.invalidate_wordbooks()` by wordbook CRUD + word create/delete), `shuffle_cache` ((book_id, tag_ids, tag_match, seed) → shuffled id sequence — tag_match 直接使用 `TagMatch` 枚举分量（common 不重复字符串化）；cap `SHUFFLE_CACHE_CAP`, cleared on word create/delete/update_tags/batch_tag), and `import_cache` (导入预览会话：token → book_id + 全量行 typed 数据 `Arc<Vec<ImportRowData>>`（跨请求缓存不中继 serde_json 字节）+ 创建时间；TTL/容量由 `config.import` 控制，后台清理任务按 `cache_cleanup_secs` 扫描；导入执行时一次性消费)。Locks are `parking_lot::Mutex`, never held across `.await`（async fn 内必须用块作用域限定 guard，否则 Future 非 Send 无法作 axum handler）。
...
- 标签筛选（纸质/列表两模式共享，按书持久化）：WordbookDetail 持有 `filterTagIds`（`qw_filter_tags_${bookId}`）+ `filterMatch: 'and'|'or'`（`qw_filter_match_${bookId}`，默认 and）；筛选面板（导航「标签」按钮下拉，`w-72 max-w-[calc(100vw-2rem)]`）顶部为「全部匹配/任一匹配」分段开关（`changeTagMatch` 清缓存重载第 1 页，打乱状态保持），下方说明文字随模式切换；后端 `words.list`/`words.query` 接受 `tag`（逗号分隔 id）+ `tag_match`（and/or，缺省 and），`repo.rs::with_tag_filter` 双语义（And = GROUP BY + `HAVING COUNT(DISTINCT tag_id)=N` 交集子查询；Or = `word_id IN (SELECT word_id FROM word_tag WHERE tag_id IN (...))` 并集子查询，word_tag 复合主键无重复行故无需 DISTINCT）；前端 `api.ts` 仅在 `tag` 非空时拼 `tag_match`。
- PaperBookView data flow (scroll mode): `WordbookDetail` holds `pages: Array<{ d: Page<Word>; pageNo: number }>` (loaded pages, ascending); page cache `Map<"${bookId}:${page}:${size}:${seed}:${tagMatch}:${tagIds}", Page<Word>>` in `useRef` + `fetchPageRaw` (cache hit + in-flight dedupe via `inflight` map) + `fetchPage` (seq guard for stale responses); `loadFrom(p)` rebuilds from page p with prefetch of next 2 pages, `loadMore(nextPageNo, totalPages)` appends on sentinel trigger; mutations clear cache and reload from `pages[0]`; page number persisted as first loaded page (`qw_page_${bookId}`).
