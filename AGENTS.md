...
- **Closed sets are enums:** query params (`order`, `seed`, `sort`, `tag_match`) are parsed once at the router boundary into `WordOrder` / `SortField` / `SortDir` / `TagMatch`; business logic matches enum variants, never string literals. `SortField`/`SortDir`/`TagMatch` use `strum::EnumString` (declarative mapping); `WordOrder` keeps a handwritten parse because `Random(String)` carries a payload strum cannot express. `TagMatch` (and/or) 走 `tag_match` 查询参数，缺省 `and`（向后兼容）；`TagMatch::cache_code()` 返回 "and"/"or" 字符串作为洗牌缓存 key 分量（common 层不依赖 business 类型）。
