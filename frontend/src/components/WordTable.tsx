...
import { words, type Page, type Tag, type TagFilter, type Word } from '../api'
...
export default function WordTable({
  ...
  const prevTagRef = useRef<{ tagFilter: TagFilter }>({ tagFilter })
  // 标签 id → 名称映射（行内 chips）
  ...
  // 查询加载：搜索词 / 排序 / 页码 / 标签筛选（组数组）/ 数据变更时重新请求
  useEffect(() => {
    if (prevTagRef.current.tagFilter !== tagFilter) {
  ...
  }, [bookId, debouncedQ, sort, order, page, refreshKey, tagFilter])
